...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-2xl">
      <div>
        <h1>Settings</h1>
        <p className="text-slate-500 text-sm mt-0.5">Manage your workspace preferences</p>
      </div>
      ...
      <div className="card p-6 space-y-4">
        ...
        <div className="space-y-3">
          <div>
            <p className="text-xs text-slate-400 uppercase font-medium tracking-wide">Name</p>
            <p className="text-sm text-slate-800 mt-0.5">{user?.name}</p>
          </div>
          <div>
      ...
      <div className="card p-6 space-y-4">
        ...
        <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 border border-slate-200">
          ...
          <p className="text-sm text-slate-600">
            The AI provider is configured via the <code className="bg-slate-200 px-1 rounded text-xs">AI_PROVIDER</code> environment variable on the server.
            Set <code className="bg-slate-200 px-1 rounded text-xs">AI_PROVIDER=mock</code> for demo mode (no API key required),
            or <code className="bg-slate-200 px-1 rounded text-xs">AI_PROVIDER=openai</code> with an <code className="bg-slate-200 px-1 rounded text-xs">OPENAI_API_KEY</code> for real extraction.
          </p>
        </div>
      </div>
      ...
      <div className="card p-6 space-y-3">
        ...
        <p className="text-sm text-slate-500">Version 1.0.0 · AI-Powered Clinical Information Intelligence</p>
